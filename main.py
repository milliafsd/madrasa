...
# CSS میں پرنٹ کے لیے مخصوص سیٹنگز شامل کی گئی ہیں
st.markdown("""
<style>
    ...
    /* Print Styles */
    @media print {
        body {direction: rtl; font-family: 'Jameel Noori Nastaleeq', Arial, sans-serif;}
        .stSidebar, header, .stButton, .stDownloadButton, .main-header { display: none !important; }
...
if not st.session_state.logged_in:
    ...
    # ================= ماہانہ رزلٹ کارڈ اور پرنٹ کی سہولت =================
        ...
        if s_list:
            ...
            sel_s = sc.selectbox("طالب علم منتخب کریں", s_list)
            date1, date2 = d1c.date_input("تاریخ آغاز", date.today().replace(day=1)), d2c.date_input("تاریخ اختتام", date.today())
            
            # پرنٹ کے لیے کالمز کو آپ کی مرضی کے مطابق شامل کیا گیا ہے
            query = f"""SELECT 
                        r_date as تاریخ, 
                        attendance as حاضری,
                        ...
                        FROM hifz_records 
                        WHERE s_name='{sel_s}' AND r_date BETWEEN '{date1}' AND '{date2}'
                        ORDER BY r_date ASC"""
            
            res_df = pd.read_sql_query(query, conn)

            if not res_df.empty:
                # سکرین پر دکھانے کے لیے
                st.dataframe(res_df, use_container_width=True, hide_index=True)
                
                # پرنٹ کے لیے HTML ٹیبل کی تیاری
                html_table = res_df.to_html(index=False, classes='print-table', border=1, justify='center')
                
                print_html = f"""
                <div class="print-container" style="display:none; direction: rtl; text-align: right;">
                    <h2 style="text-align: center;">جامعہ ملیہ اسلامیہ - ماہانہ تعلیمی رپورٹ</h2>
                    <p><b>طالب علم کا نام:</b> {sel_s} &nbsp;&nbsp;&nbsp; <b>تاریخ:</b> {date1} سے {date2}</p>
                    {html_table}
                ...
                # پرنٹ بٹن
                if col_btn1.button("🖨️ رپورٹ پرنٹ کریں"):
                    js_print = """
                    <script>
                        window.print();
                    </script>
        ...
        if not att_df.empty: 
            ...
            st.download_button("📥 ڈاؤن لوڈ ریکارڈ", convert_df_to_csv(att_df), "attendance.csv", "text/csv")
    ...
    # ================= حاضری کا درست نظام (Check-in / Check-out) =================
    elif m == "🕒 میری حاضری":
        st.header("🕒 آمد و رخصت (حاضری)")
        today_date = date.today()
        
        # چیک کریں کہ آج کا ریکارڈ موجود ہے یا نہیں
        ...
        elif record and record[1] is None: # اگر آمد لگی ہے اور رخصت نہیں لگی
