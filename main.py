...
def sanitize_input(text: str, max_length: int = 500) -> str:
    ...
    # SQL injection کے خلاف بنیادی صفائی
...
def verify_login(username: str, password: str):
    ...
    # صرف hashed پاسورڈ چیک کریں
    res = conn.execute(
        "SELECT * FROM teachers WHERE name=? AND password=? AND is_active=1",
        (username, hashed)
    ).fetchone()
    
    # پرانا plain password بھی سپورٹ کریں (migration)
    if not res:
        ...
        if st.button("▶ داخل ہوں", use_container_width=True):
            if u and p:
                ...
                if res == "locked":
                    ...
                    st.error(f"⛔ بہت زیادہ غلط کوششیں۔ {remaining} منٹ بعد کوشش کریں۔")
                elif res:
...
if selected == "📊 ایڈمن ڈیش بورڈ" and st.session_state.user_type == "admin":
    ...
    active_students = conn.execute("SELECT COUNT(*) FROM students WHERE exit_date IS NULL OR exit_date=''").fetchone()[0]
    ...
    if dept == "حفظ":
        ...
        if not students:
            ...
            for sid, s, f in students:
                ...
                # پہلے چیک کریں کہ آج کا ریکارڈ موجود ہے
                conn = get_db_connection()
